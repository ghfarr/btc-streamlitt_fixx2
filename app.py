"""
Bitcoin Price Prediction Dashboard — Streamlit App
...
st.set_page_config(
    page_title="Bitcoin Price Prediction Dashboard",
    page_icon="₿",
    layout="wide",
...
st.sidebar.caption("Bitcoin price prediction — ML + sentiment Twitter")
...
st.sidebar.markdown(
    """
**Setup**
- Train (80%): 2021-01 → 2023-12
- Validation (10%): 2024-01 → 2024-05
- Test (10%): 2024-05 → 2024-09
...
def page_overview():
    ...
    st.markdown(
        "Menganalisis prediksi harga **Bitcoin** menggunakan tiga skenario data: "
    ...
    c4.metric("Skenario", metrics["scenario"].nunique())

    st.divider()

    # Key chart: BTC price
    st.subheader("Harga BTC sepanjang periode (2021–2024)")
    fig = go.Figure()
    ...
    fig.add_vrect(x0="2024-01-01", x1="2024-05-16", fillcolor="LightSkyBlue",
                  opacity=0.2, line_width=0, annotation_text="Validation", annotation_position="top left")
    fig.add_vrect(x0="2024-05-17", x1="2024-09-30", fillcolor="LightSalmon",
    ...
    best = (
        ...
        .sort_values(["scenario", "horizon", "RMSE"])
        .groupby(["scenario", "horizon"])
        .head(1)
        .reset_index(drop=True)
    )
    st.dataframe(
        best[["scenario", "horizon", "model", "RMSE", "MAE", "MAPE"]]
...
def page_data_sentiment():
    ...
    with tab_btc:
        ...
        fig = px.line(btc, x="date", y=col, title=f"BTC {col} (2021-2024)")
    ...
    with tab_tweets:
        ...
        with f2:
            sent_filter = st.select_slider("Sentimen (compound)",
                                           options=["semua", "negatif", "netral", "positif"],
                                           value="semua")
...
def page_features():
    ...
    scenario = st.selectbox("Scenario", ["Historical", "Sentiment", "Combined"])
    fcols = scenario_features[scenario]

    st.markdown(f"Scenario **{scenario}** menggunakan **{len(fcols)} fitur**.")
    st.code("\n".join(fcols))
    ...
    fig = px.bar(
        corr.reset_index().rename(columns={"index": "feature", target_pick: "corr"}),
...
def page_predictions():
    ...
    scenario    = c1.selectbox("Scenario", ["Historical", "Sentiment", "Combined"])
    horizon     = c2.selectbox("Horizon", ["t+1", "t+30"], index=0)
    split       = c3.selectbox("Split", ["test", "validation"], index=0)
    models_pick = c4.multiselect("Model", pred_models, default=pred_models)
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=actual["date"], y=actual["actual"],
    ...
    for m in models_pick:
        ...
        fig.add_trace(go.Scatter(
            x=sm["date"], y=sm["y_pred"], mode="lines", name=m,
            line=dict(color=MODEL_COLORS.get(m, "#999"), width=1.5), opacity=0.85
        ))

    fig.update_layout(
        title=(f"{scenario} | {horizon} | {split.title()} "
    ...
    summary = (
        err_df.groupby("model")["abs_error"]
        .agg(["mean", "median", "max"])
        .round(2).reset_index()
    )
    summary.columns = ["Model", "MAE", "Median |Error|", "Max |Error|"]
...
def page_metrics():
    ...
    scenarios_pick = st.multiselect(
        "Scenario", ["Historical", "Sentiment", "Combined"],
    ...
    st.dataframe(
        metrics.style.format({"RMSE": "{:.2f}", "MAE": "{:.2f}", "MAPE": "{:.2f}%"}),
        use_container_width=True, hide_index=True
    )
    ...
    metric     = col_m.radio("Metric", ["RMSE", "MAE", "MAPE"], horizontal=True)
    split_pick = col_s.radio("Split", ["test", "validation"], horizontal=True)

    c1, c2 = st.columns(2)
    for i, h in enumerate(["t+1", "t+30"]):
        sub = metrics[(metrics["horizon"] == h) & (metrics["split"] == split_pick)]
        fig = px.bar(
            sub, x="model", y=metric, color="scenario", barmode="group",
            color_discrete_map={
                "Historical": "#f7931a",
                "Sentiment":  "#9467bd",
                "Combined":   "#17becf",
    ...
    st.divider()
    st.subheader("Best Model per Skenario & Horizon (Test RMSE)")
    best = (
        metrics[metrics["split"] == "test"]
        .sort_values(["scenario", "horizon", "RMSE"])
        .groupby(["scenario", "horizon"])
...
def page_scenario_comparison():
    ...
    sub = metrics[(metrics["split"] == split) & (metrics["horizon"] == horizon)]
    ...
    fig.update_layout(height=450, margin=dict(l=10, r=10, t=50, b=10),
                      xaxis_title="Model", yaxis_title=metric)
    st.plotly_chart(fig, use_container_width=True)

    # Actual vs Predicted semua skenario
    ...
    for sc in ["Historical", "Sentiment", "Combined"]:
        ...
        fig2.add_trace(go.Scatter(
            ...
            mode="lines", name=sc,
            line=dict(color=sc_colors[sc], width=1.6), opacity=0.85
        ))

    fig2.update_layout(
        height=480, xaxis_title="Tanggal", yaxis_title="BTC Close (USD)",
    ...
    heat = sub.pivot_table(index="scenario", columns="model", values=metric).round(2)
    fig3 = px.imshow(
        heat, text_auto=".2f", color_continuous_scale="RdYlGn_r",
        title=f"Heatmap {metric} — Horizon {horizon} · {split.title()}", aspect="auto",
    )
